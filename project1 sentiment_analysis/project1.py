...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    """
    # Your code here
    test = label * (np.dot(theta,np.transpose(feature_vector)) + theta_0)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    classifier = labels*(np.dot(theta,np.transpose(feature_matrix)) + theta_0)
    HL = np.average(np.where(classifier>=1, 0, 1-classifier))
...
def perceptron_single_step_update(
    ...
    """
    # Your code here
    if label*(np.dot(current_theta, np.transpose(feature_vector)) + current_theta_0) <= 0:
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.array(np.zeros(feature_matrix.shape[1],))
    theta_0 = 0
    sumtheta = 0
    sumtheta_0 = 0
...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    if label*(np.dot(current_theta, np.transpose(feature_vector)) + current_theta_0) <= 1:
